  ...
  return (
    <header className="sticky top-0 z-100 flex items-center justify-between border-b border-rule/50 bg-bg/85 px-5 py-4 backdrop-blur-[10px] backdrop-saturate-150 sm:px-7 sm:py-4.5 md:px-10 lg:px-[clamp(28px,4vw,56px)] 2xl:px-20">
      ...
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        className="relative z-110 inline-flex h-11 w-11 items-center justify-center rounded-lg border border-rule/60 text-ink md:hidden"
      >
      ...
      {open && (
        <div
          ...
          className="fixed inset-0 z-105 flex flex-col bg-bg px-6 pt-20 pb-10 md:hidden"
        >
          ...
          <a
            href="#contact"
            onClick={() => setOpen(false)}
            className="mt-8 inline-flex items-center justify-center gap-2 self-start rounded-full bg-ink px-6 py-3.5 text-[15px] font-medium !text-bg"
          >
            Contact
            <svg
              width="16"
